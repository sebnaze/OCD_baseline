...
atlas_suffix = {'schaefer100_tianS1':'MNI_lps_mni.nii.gz', \
                ...
                'schaefer400_tianS1':'MNI_lps_mni.nii.gz', \
...
class Atlaser:
    ...
    def create_atlas_df(self, subctx=['Hipp', 'Thal', 'Put', 'Caud', 'Amyg', 'Accumb', 'Pal']):
        """ create a pandas DataFrame of the atlas """
        #df_atlas = pd.DataFrame(columns=['node_ids', 'node_names', 'hemi', 'network', 'roi'])
        lines = []
        for i,name in enumerate(self.node_names):
            ...
            sname = name.split('_')
            line['hemi'] = sname[0]
            subnet = [sub for sub in subctx if sub in sname[1]]
            if subnet!=[]:
                line['network'] = subnet[0]
                line['roi'] = sname[1]
            else:
                line['network'] = sname[1]
                if sname[2].isnumeric():
                    line['roi'] = 'SM'
                else:
                    line['roi'] = sname[2]
            line['label'] = name
    ...
    def get_df_centroids(self):
        """ export rois centroids in pandas DataFrame """
        lines = []
        for m,n_id in enumerate(self.node_ids):
             i_s, j_s, k_s = np.where(self.atlas_img.get_fdata()==n_id)
             i,j,k = np.mean(i_s), np.mean(j_s), np.mean(k_s)
             x,y,z = nilearn.image.coord_transform(i,j,k, affine=self.atlas_img.affine)
             lines.append({'id':n_id, 'centroid':(x,y,z)})
        return pd.DataFrame(lines)
