...
def get_df_voi_corr():
    ...
    with open(os.path.join(proj_dir, 'postprocessing', 'df_voi_corr_replicate_T_15022023.pkl'), 'rb') as f:
        df_corr_voi = pickle.load(f)
    return df_corr_voi

def plot_corr_voi(df, args=None):
    """ plot correlation (linear regression + pearson correlation and p-value) between FC and behavioral scores (Y-BOCS) """
    coh_col = {'controls': 'blue', 'patients':'orange'}
    ...
    for behav in behavs:
        ...
        for i, pathway in enumerate(df.pathway.unique()):
            ...
            data = df[df['pathway']==pathway]
            ax = sbn.regplot(data=data, x=behav, y='corr', color=coh_col[cohort])
        ...
        plt.tight_layout()
        if args.save_figs:
            plt.savefig(os.path.join(proj_dir, 'img', 'FC_{}_relation_.pdf'.format(behav)), transparent=True)
...
def plot_drugfree(args):
    ...
    sbn.swarmplot(data=df_corr_voi, y='corr', x='pathway', hue='meds', size=4, palette=sbn.color_palette('pastel'))
    plt.legend(bbox_to_anchor=(1.02, 1), loc='upper left', borderaxespad=0)

    if args.save_figs:
        plt.savefig(os.path.join(proj_dir, 'img', 'FC_drugfree_.pdf'))
